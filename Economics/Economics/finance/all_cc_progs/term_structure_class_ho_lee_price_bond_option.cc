...
double price_european_call_option_on_bond_using_ho_lee(term_structure_class* initial, const double& delta, const double& pi,
    ...
    for (int i=0;i<=T;++i){
		values[i] = _max(0.0, bonds_price(vec_cf[T + 1].times, vec_cf[T + 1].cash_flows, hl_tree[T + 1][i]) - K);
    };
    for (int t=T;t>=0;--t){
	vector<double> values_this(t+1);
	for (int i=0;i<=t;++i){ 
		double val = 0.0f;
		if ((t < hl_tree.size()) &&( i < hl_tree[t].size()))
			val = hl_tree[t][i].d(1);
		if (i+1 < values.size() )
			values_this[i]=(pi*values[i+1]+(1.0-pi)*values[i])*val; 
	};
	values=values_this;
    };
    return values[0];
